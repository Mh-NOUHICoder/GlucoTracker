...
export default function InitialLoader({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Show loader and then trigger fade out
    const timer = setTimeout(() => {
      setIsFading(true);
      setTimeout(() => {
        setShowLoader(false);
      }, 500); // 500ms fade out duration
    }, 3500); // Show for 7 seconds on every load
  ...
  return (
    <>
      {showLoader && (
        <div 
          className={`fixed inset-0 z-[9999] flex items-center justify-center bg-medical-black transition-opacity duration-500 ease-in-out ${
